...
//     in IE returns signed numbers when used on full words)
  ...
  */
  ...
  function changeAttrRelWithAbs(node,attr){
     var abs=node[attr];
     if(node.getAttribute(attr)!=abs){
      node.setAttribute(attr,abs);
     }  
  };
  
  function applyToAll(children,f,params){
    var i;
  ...
  this.getStyledNode=function(node) {
  	//d=node.ownerDocument;
    
    clean_active();
    cachedStyles = {}; // clean style cache
    
    applyToAll(node.querySelectorAll("[src]"),changeAttrRelWithAbs,"src");
    changeAttrRelWithAbs(node,"src");  
    applyToAll(node.querySelectorAll("[href]"),changeAttrRelWithAbs,"href");
    changeAttrRelWithAbs(node,"href");  
    applyToAll(node.querySelectorAll("[action]"),changeAttrRelWithAbs,"action");
    changeAttrRelWithAbs(node,"action");      
    applyToAll(node.querySelectorAll("a"),addTargetBlank);
    addTargetBlank(node,"href");  
    ...
    var res ="<style type=\"text/css\">\n{0}\n</style>\n{1}".format(cssText, idoc.body.innerHTML);
